...
public sealed class StudentCardViewModel : NotifyPropertyChangedBase, IDisposable
{
    ...
    public async Task LoadStudentAsync(string studentId)
    {
        ...
        try
        {
            ...
            if (Student != null)
            {
                PhotoImage = _photoService.ResolvePhotoPath(Student.Photo);
            }
    ...
    public async Task<bool> AddPhotoAsync()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(photoPath))
            {
                ...
                PhotoImage = _photoService.ResolvePhotoPath(photoPath);
                IsChanged = true;
                return true;
            }
    ...
    /// 사진 재로드 (내부용)
    /// </summary>
    private Task ReloadPhotoAsync()
    {
        PhotoImage = Student == null ? null : _photoService.ResolvePhotoPath(Student.Photo);
